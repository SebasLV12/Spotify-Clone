...
export const Pause = ({ className }) => (
  ...
  export const NextSong=()=>(
    <svg role="img" height="24" width="24" aria-hidden="true" viewBox="0 0 24 15" fill="currentColor"><path d="M12.7 1a.7.7 0 0 0-.7.7v5.15L2.05 1.107A.7.7 0 0 0 1 1.712v12.575a.7.7 0 0 0 1.05.607L12 9.149V14.3a.7.7 0 0 0 .7.7h1.6a.7.7 0 0 0 .7-.7V1.7a.7.7 0 0 0-.7-.7h-1.6z"></path></svg>
  )
...
export function Player(){
    const {currentMusic,nextMusic,previousMusic,setCurrentMusic, setNextMusic,isPlaying,setIsPlaying,volume}=usePlayerStore(state=>state)
    ...
    useEffect(()=>{
     ...
     useEffect(()=>{
        const{song,playlist,songs}=nextMusic
        if(song){
            const src=`/music/${playlist?.id}/0${song.id+1}.mp3`
            console.log(src)
            audioRef.current.src=src
            audioRef.current.volume=volume
            audioRef.current.play()
